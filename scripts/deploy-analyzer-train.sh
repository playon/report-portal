...
FAMILY=$(cat ./configs/vars-${ENVIRONMENT}.json | jq -r .AnalyzerTrain.Family)
SERVICE_NAME=$(cat ./configs/vars-${ENVIRONMENT}.json | jq -r .AnalyzerTrain.ServiceName)
TASK_ROLE=$(cat ./configs/vars-${ENVIRONMENT}.json | jq -r .AnalyzerTrain.TaskRole)
EXECUTION_TASK_ROLE=$(cat ./configs/vars-${ENVIRONMENT}.json | jq -r .AnalyzerTrain.ExecutionTaskRole)
AWS_LOGS_GROUP=$(cat ./configs/vars-${ENVIRONMENT}.json | jq -r .AnalyzerTrain.AwsLogsGroup)
...
aws ecs register-task-definition \
    ...
    --volumes "${VOLUMES}"  > ./report_portal_analyzer_train_register_task_definition_output.json

NEW_REVISION=$(cat ./report_portal_analyzer_train_register_task_definition_output.json | jq  '.taskDefinition.revision')
